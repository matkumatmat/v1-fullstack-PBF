...
import { ChevronDown, MoreHorizontal, Pencil, Trash, Eye } from 'lucide-react';
...
import { AddCustomerDialog } from '@/app/admin/print/label/components/add-customer-dialog';
...
const fetchCustomers = async (skip: number, limit: number): Promise<Customer[]> => {
  ...
  // INI ADALAH LANGKAH DEBUGGING PALING PENTING
  console.log("==============================================");
  console.log("DATA MENTAH DITERIMA DARI API (SEBELUM ZOD):");
  console.log(jsonData);
  console.log("==============================================");
  
  // Kita tetap coba validasi, tapi log di atas akan menunjukkan kebenarannya
...
const columns: ColumnDef<Customer>[] = [
  { id: 'expander', header: () => null, cell: ({ row }) => (<Button variant="ghost" size="icon" onClick={row.getToggleExpandedHandler()} className="w-8 h-8"><ChevronDown className={`h-4 w-4 transition-transform duration-200 ${row.getIsExpanded() ? 'rotate-180' : ''}`} /></Button>), },
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            ...
            <DropdownMenuItem asChild>
              <Link href={`/admin/manage/customer/${row.original.public_id}`}>
...
export function CustomersDataTable() {
  ...
  React.useEffect(() => { if (isLoading) { setExpanded({}); } }, [isLoading]);
  const tableData = queryData ?? [];
  const hasMoreData = tableData.length === pagination.pageSize;
  const table = useReactTable({
    data: tableData, columns, state: { pagination, expanded },
    onPaginationChange: setPagination, onExpandedChange: setExpanded,
    getRowCanExpand: () => true, getCoreRowModel: getCoreRowModel(),
    ...
    <Card>
      <CardHeader>
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center">
          <div><CardTitle>Manajemen Customer</CardTitle><CardDescription>Kelola semua data customer, cabang, dan lokasi di database.</CardDescription></div>
      ...
      <CardContent>
          ...
          <Table>
            ...
            <TableBody>
              {isLoading && tableData.length === 0 ? (
                Array.from({ length: 5 }).map((_, i) => (<TableRow key={`skeleton-${i}`}><TableCell colSpan={columns.length}><Skeleton className="h-10 w-full" /></TableCell></TableRow>))
                ...
                table.getRowModel().rows.map((row) => (
                  <React.Fragment key={row.id}>
                    ...
                    {row.getIsExpanded() && (
                      <TableRow><TableCell colSpan={columns.length}><CustomerSubComponent customerId={row.original.public_id} /></TableCell></TableRow>
                    )}
                  </React.Fragment>
                ))
              ) : (
        ...
        <div className="flex items-center justify-end space-x-2 py-4">
          ...
          <Button variant="outline" size="sm" onClick={() => table.nextPage()} disabled={!hasMoreData || isLoading}>Selanjutnya</Button>
        </div>
      </CardContent>
    </Card>
  );
