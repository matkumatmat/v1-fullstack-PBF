...
if TYPE_CHECKING:
    ...
    from ..product import Allocation, Batch

class Rack(BaseModel):
    __tablename__ = 'racks'
    code: Mapped[str] = mapped_column(String(50), unique=True, nullable=False, index=True)
    status: Mapped[RackStatusEnum] = mapped_column(
        SQLAlchemyEnum(RackStatusEnum, name="rack_status_enum", create_type=False),
        default=RackStatusEnum.ACTIVE,
        nullable=False
    )    
    ...
    item: Mapped[Optional['RackItem']] = relationship(back_populates='rack', cascade="all, delete-orphan")
    def __repr__(self) -> str:
        return f"<Rack(id={self.id}, code='{self.code}', status='{self.status.value}')>"
