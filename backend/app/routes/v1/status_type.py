...
def create_new_status_type(
    status_type: StatusTypeCreate, db: Session = Depends(get_db_session)
):
    return create_status_type(db=db, status_type=status_type)

@router.get("/{status_type_id}", response_model=StatusType)
def read_status_type(status_type_id: int, db: Session = Depends(get_db_session)):
...
def delete_existing_status_type(
    status_type_id: int, db: Session = Depends(get_db_session)
):
