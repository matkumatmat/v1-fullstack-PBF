...
def create_new_allocation_type(
    allocation_type: AllocationTypeCreate, db: Session = Depends(get_db)
):
...
def read_allocation_type(allocation_type_id: int, db: Session = Depends(get_db)):
...
def delete_existing_allocation_type(
    allocation_type_id: int, db: Session = Depends(get_db)
):
