...
def create_new_priority_level(
    priority_level: PriorityLevelCreate, db: Session = Depends(get_db_session)
):
...
def read_priority_level(priority_level_id: int, db: Session = Depends(get_db_session)):
...
def delete_existing_priority_level(
    priority_level_id: int, db: Session = Depends(get_db_session)
):
