...
def create_new_product_price(
    product_price: ProductPriceCreate, db: Session = Depends(get_db_session)
):
    return create_product_price(db=db, product_price=product_price)

@router.get("/{product_price_id}", response_model=ProductPrice)
def read_product_price(product_price_id: int, db: Session = Depends(get_db_session)):
...
def delete_existing_product_price(
    product_price_id: int, db: Session = Depends(get_db_session)
):
