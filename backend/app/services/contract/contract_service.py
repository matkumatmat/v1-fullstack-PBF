...
class TenderContractService(CRUDService):
    ...
    async def create(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if contract_number:
            await self._validate_unique_field(TenderContract, 'contract_number', contract_number,
        ...
        # Create contract
        contract_data = await super().create(data)
        
        # Send notification
        await self._send_notification('CONTRACT_CREATED', ['admin', 'sales_team'], {
    ...
    async def update(self, entity_id: int, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        contract = await self._get_or_404(TenderContract, entity_id)
        
        # Restrict updates if contract has reservations
        if await self._contract_has_reservations(entity_id):
    ...
    async def activate_contract(self, contract_id: int) -> Dict[str, Any]:
        """Activate contract"""
        ...
        # Send notification
        await self._send_notification('CONTRACT_ACTIVATED', ['admin', 'sales_team'], {
    ...
    async def suspend_contract(self, contract_id: int, reason: str = None) -> Dict[str, Any]:
        """Suspend contract"""
        contract = await self._get_or_404(TenderContract, contract_id)
        ...
        # Send notification
        await self._send_notification('CONTRACT_SUSPENDED', ['admin', 'sales_team'], {
            'contract_id': contract_id,
            'contract_number': contract.contract_number,
            'reason': reason
        })
    ...
    async def complete_contract(self, contract_id: int) -> Dict[str, Any]:
        """Complete contract"""
        contract = await self._get_or_404(TenderContract, contract_id)
        ...
        result = await self.db_session.execute(
            select(func.count(ContractReservation.id)).filter(
                and_(
        ...
        pending_reservations = result.scalar_one()
        ...
        # Send notification
        await self._send_notification('CONTRACT_COMPLETED', ['admin', 'sales_team'], {
            'contract_id': contract_id,
            'contract_number': contract.contract_number
        })
    ...
    async def get_by_contract_number(self, contract_number: str) -> Dict[str, Any]:
        """Get contract by contract number"""
        result = await self.db_session.execute(
            select(TenderContract).filter(TenderContract.contract_number == contract_number)
        )
    ...
    async def get_active_contracts(self, include_expiring: bool = True, 
        ...
        for contract in contracts:
            ...
            contract_data['reservation_summary'] = await self._get_contract_reservation_summary(contract.id)
    ...
    async def get_contract_performance_report(self, contract_id: int) -> Dict[str, Any]:
        """Get contract performance report"""
        contract = await self._get_or_404(TenderContract, contract_id)
        
        # Get all reservations
        reservations_result = await self.db_session.execute(
        ...
        allocations_result = await self.db_session.execute(
            select(Allocation).filter(Allocation.tender_contract_id == contract_id)
        )
    ...
    async def _contract_has_reservations(self, contract_id: int) -> bool:
        """Check if contract has any reservations"""
        result = await self.db_session.execute(
            select(func.count(ContractReservation.id)).filter(ContractReservation.contract_id == contract_id)
        )
        return result.scalar_one() > 0
    
    async def _get_contract_reservation_summary(self, contract_id: int) -> Dict[str, Any]:
