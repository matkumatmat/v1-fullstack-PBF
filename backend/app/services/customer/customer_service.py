...
class CustomerService(CRUDService):
    ...
    async def _validate_customer_type(self, customer_type_id: int):
        """Validate customer type exists"""
        result = await self.db_session.execute(
            select(CustomerType).filter(CustomerType.id == customer_type_id)
        )
        if not result.scalars().first():
            raise ValidationError(f"Customer type with ID {customer_type_id} not found")
    
    async def _validate_sector_type(self, sector_type_id: int):
        """Validate sector type exists"""
        result = await self.db_session.execute(
            select(SectorType).filter(SectorType.id == sector_type_id)
        )
        if not result.scalars().first():
